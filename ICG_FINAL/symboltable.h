...
entry_t *create_entry( char *lexeme, int value, int data_type, int lineno, int storage, char* character_constant)
{
	entry_t *new_entry;
	...
	if(character_constant == NULL)
	{
		...
		new_entry->char_value = NULL;
	}
	else
	{
		new_entry->value = -1;
		new_entry->char_value =character_constant;
	}	
...
entry_t* insert( entry_t** hash_table_ptr, char* lexeme, int value, int data_type, int lineno,int storage,char* character_constant)
{
	...
	new_entry = create_entry( lexeme, value, data_type,lineno,storage,character_constant); // Create an entry using the <lexeme, token> pair
...
void display_symbol_table(entry_t** hash_table_ptr)
{
  ...
  printf(" %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n","lexeme","data-type","storage","array_dimension","lineno","num_params","param_list");
	...
	for( i=0; i < HASH_TABLE_SIZE; i++)
	{
		...
		{
			printf(" %-20s %-20d %-20d %-20d %-20d", traverser->lexeme, traverser->data_type, (int)traverser->storage ,traverser->array_dimension,traverser->lineno);
